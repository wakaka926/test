<template>
  ...
  import { loadPracticeRecord, updatePracticeRecordWhileSubmitAnswer, updatePracticeRecordWhileLeavePage } from '@/api/study/incorrect'
  ...
  function restorePracticeState(data) {
    ...
    if (!data || !data.record || !data.details) {
  ...
  async function loadPracticeRecordData() {
    try {
      ...
      console.log('加载练习记录，参数:', { studentId})
  
      const response = await loadPracticeRecord(studentId)
