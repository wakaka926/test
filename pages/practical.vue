<template>
    <view class="practical-container">
      <!-- 顶部栏：返回 + 科目/练习类型 -->
      <view class="practical-header">
        <view class="header-center">
          <text class="practical-title">{{ typeName }}</text>
        </view>
  ...
  import { recordWrongQuestion, loadPracticalRecord, updatePracticalRecordWhileSubmitAnswer, updatePracticalRecordWhileLeavePage } from '@/api/study/practicalRecord'
  
  const type = ref(null) // null | true_false | single_choice
  const practicalBankId = ref(null)
  ...
  // 练习记录相关变量
  const practicalRecord = ref(null) // 练习记录
  const practicalRecordId = ref(null) // 练习记录ID
  ...
  // 恢复练习状态
  function restorePracticalState(data) {
    console.log('restorePracticalState:', data)
    if (!data || !data.practicalRecord || !data.details) {
      console.log('没有练习记录数据，开始新的练习')
      return false
    }
  
    const record = data.practicalRecord
    const details = data.details
    const savedIndex = data.currentQuestionIndex
    console.log('savedIndex:', savedIndex)
    // 保存练习记录信息
    practicalRecord.value = record
    practicalRecordId.value = record.id
    ...
    console.log('练习状态已恢复:', {
      practicalRecordId: practicalRecordId.value,
  ...
  // 加载练习记录
  async function loadPracticalRecordData() {
    try {
      const bankId = practicalBankId.value
      ...
      const practicalType = (type.value === 'all' || type.value === null) ? null : type.value
  
      console.log('加载练习记录，参数:', { studentId, bankId, practicalType })
  
      const response = await loadPracticalRecord(studentId, bankId, practicalType)
  
      if (response.code === 200 && response.data) {
        return restorePracticalState(response.data)
      } else {
        console.log('未找到练习记录')
        return false
      }
  ...
  async function loadQuestions() {
    ...
    try {
      loading.value = true
      const response = await loadPracticalRecord(studentId, practicalBankId.value, type.value)
  ...
  async function submitCurrentQuestion() {
    ...
    // 如果有练习记录ID，更新练习记录
    if (practicalRecordId.value && questionId && userAnswer) {
      try {
        await updatePracticalRecordWhileSubmitAnswer(
          practicalRecordId.value,
          questionId,
          userAnswer,
          timeSpentSeconds,
          isCorrect,
        ...
        console.log('练习记录更新成功:', {
          practicalRecordsId: practicalRecordId.value,
          questionId,
          userAnswer,
          timeSpentSeconds,
          isCorrect,
  ...
  onLoad(async (opts) => {
    ...
    practicalBankId.value = opts?.practicalBankId || null
  
    // 加载题目列表
    await loadQuestions()
  
    // 加载练习记录并恢复状态
    const hasRecord = await loadPracticalRecordData()
  ...
  onUnload(() => {
    ...
    // 如果有练习记录ID，保存当前状态
    if (practicalRecordId.value) {
      // 异步调用更新接口，但不等待完成（因为页面正在卸载）
      updatePracticalRecordWhileLeavePage(practicalRecordId.value).then(() => {
        console.log('离开页面时练习记录保存成功:', practicalRecordId.value)
      }).catch((error) => {
